...
Uso:
  ...
  scripts\setup_and_run.ps1 run-img <imagen> [-Out out] [-Lite] [-Cpu] [-Score 0.5] [-MaxSide 1280]
  scripts\setup_and_run.ps1 run-dir <carpeta> [-Out out] [-Lite] [-Cpu] [-Score 0.5] [-MaxSide 1280]
  scripts\setup_and_run.ps1 run-vid <video> [-Out out] [-Lite] [-Cpu] [-Fps 1.0] [-Score 0.5] [-MaxSide 1280]
  scripts\setup_and_run.ps1 run-cam [-Out out] [-Lite] [-Cpu] [-Fps 1.0] [-MaxSide 1280]
  scripts\setup_and_run.ps1 docker-build [-Tag objdesc:cpu]
  scripts\setup_and_run.ps1 docker-run <run-img|run-dir|run-vid> <ruta> [-Out out] [-Lite] [-Cpu] [-Fps 1.0] [-Score 0.5] [-MaxSide 1280] [-Tag objdesc:cpu]
"@
}
...
det = pipeline('object-detection', model=det_model, device=-1)
cap = pipeline('image-to-text', model=cap_model, device=-1)
...
# ---------- Parse helpers (sin param dentro del switch) ----------
...
switch ($cmd) {
  ...
  "run-img" {
    ...
    $opts = Parse-Common(@{ Out="out"; Lite=$false; Cpu=$false; Score=0.5; MaxSide=0 })
  ...
  "run-vid" {
    ...
    $opts = Parse-Common(@{ Out="out"; Lite=$false; Cpu=$false; Score=0.5; MaxSide=0; Fps=1.0 })
    Assert-File $AppEntry; Activate-Venv
  ...
  "run-cam" {
    $opts = Parse-Common(@{ Out="out"; Lite=$false; Cpu=$false; MaxSide=0; Fps=1.0 })
    Assert-File $AppEntry; Activate-Venv
  ...
  "docker-run" {
    ...
    $opts = Parse-Common(@{ Out="out"; Lite=$false; Cpu=$true; Score=0.5; MaxSide=0; Fps=1.0; Tag="objdesc:cpu" })
    $vol = "$($PWD.Path):/app"   # FIX: evitar $PWD:/app
